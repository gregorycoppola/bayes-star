...
use super::objects::{PredicateConjunction, PropositionConjunction};
    ...
    for (i, term) in conjunction.terms.iter().enumerate() {
        ...
        match fact_db.get_proposition_probability(term) {
            Ok(term_prob_opt) => {
                ...
                trace!("Term probability for term {}: {}", term.predicate.search_string(), term_prob);
                product *= term_prob;
                trace!("Updated min probability after term {}: {}", term.predicate.search_string(), product);
            },
            Err(e) => {
