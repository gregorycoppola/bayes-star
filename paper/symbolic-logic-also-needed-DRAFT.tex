...
\subsection{Importance of LLM's}
...
\subsection{Short History of LLM's}
Bengio et al. \citep{bengio2003neural} introduced a novel neural network-based approach to language modeling, significantly impacting subsequent developments in natural language processing.
Sutskever et al. \citep{sutskever2014sequence} presented a breakthrough in sequence-to-sequence learning, revolutionizing machine translation and other sequence-based tasks in AI.
Vaswani et al. \citep{vaswani2017attention} introduced the Transformer model, fundamentally changing the landscape of neural network design for language processing.
Devlin et al. \citep{devlin2018bert} proposed BERT, setting new standards in natural language understanding tasks.
Radford et al. \citep{radford2018improving} advanced language understanding through generative pre-training, paving the way for sophisticated language models.
Radford et al. \citep{radford2019language} demonstrated the versatility of language models as unsupervised multitask learners.
Brown et al. \citep{brown2020language} showed that language models can be effective few-shot learners, expanding possibilities with minimal training data.
Kaplan et al. \citep{kaplan2020scaling} explored scaling laws for neural language models, providing insights into model size and performance.
...
Fine-tuning is the strategy that {\em ChatGPT} originally used to constrain the generative model,
...
product, especially a {\em search engine}.
...
We list {\bf trusted sources} as its own element, for clarity.
However, the {\em trusted sources} strategy is ultimately a {\em RAG} strategy, so the same arguments would apply.
...
However, from an information retrieval system, it is very simple: each entity is associated with a unique identifier (say a string).
...
In this case, we can side-step philosophico-mystical debates by appealing to a simple notion of ``truth'' that the LLM is not currently offering us, which is that: we want the LGM to be able to accurately represent what was in its training set.
In other words, if a LGM tells us that a certain piece of information is in its data set, we should be able to find it there.
In fact, an LGM should be able to tell us why it believes a certain statement, and provide links to all of the data that was in its training set.

\subsection{Semantic Roles}
For us, a \emph{predicate}, which is also a node in the probability graph, is a \emph{key-value} tuple.
The keys come from a fixed set, called \emph{roles}.
That is, while the number of different \emph{languages} is not bounded, and, for any given language, the number of words or morphemes is not bounded, the set of roles \emph{is} finite, bounded, and fixed, for all languages.
